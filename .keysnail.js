...
plugins.options["kkk.sites"] = ["^https?://([0-9a-zA-Z]+\\.)?github\\.com/",
                                "^https?://([0-9a-zA-Z]+\\.)?reddit\\.com/"];

plugins.options["hok.local_queries"] = [
    [".*", ".title"]
]

shell.add(["ya-maps-remove-junk"], "removes junk on yandex maps", function() {
...
key.setGlobalKey([['C-x', 'k'], ['C-j', 'w']], function (ev) {
    BrowserCloseTabOrWindow();
}, 'Close tab / window');

key.setGlobalKey([['C-x', 'K'], ['C-M-w']], function (ev) {
    closeWindow(true);
}, 'Close the window');

key.setGlobalKey([['C-x', 'n'], ['M-n']], function (ev) {
    OpenBrowserWindow();
}, 'Open new window');
...
key.setGlobalKey(['C-j', 'c'], function (ev) {
    BrowserOpenTab();
}, 'New tab');

key.setGlobalKey([['C-j', 'n'], ['C-M-l'], ['C-M-n']], function (ev) {
    getBrowser().mTabContainer.advanceSelectedTab(1, true);
}, 'Select next tab');

key.setGlobalKey([['C-j', 'p'], ['C-M-h'], ['C-M-p']], function (ev) {
    getBrowser().mTabContainer.advanceSelectedTab(-1, true);
}, 'Select previous tab');
...
key.setGlobalKey([['M-s', 's']], function (ev) {
...
key.setGlobalKey(['C-c', 'b'], function (aEvent, aArg) {
    ext.exec("hok-start-background-mode", aArg);
}, 'HoK - Background hint mode', true);

key.setGlobalKey(['C-c', 'y'], function (aEvent, aArg) {
