...
// Block reddit key bindings, they interfere with keysnail
plugins.options["kkk.sites"] = ["^https?://([0-9a-zA-Z]+\\.)?reddit\\.com/"];
//}}%PRESERVE%
// ========================================================================= //
...
// ============================= Key bindings ============================== //
key.setGlobalKey(['C-c', 'g'], function (aEvent, aArg) {
...
key.setViewKey([['C-x', 'C-h']], function (ev) {
                BrowserBack();
            }, 'Back');

key.setViewKey([['C-x', 'C-s']], function (ev) {
                BrowserForward();
            }, 'Forward');
...
key.setEditKey('M-c', function (ev) {
    command.copyRegion(ev);
}, 'Copy selected text');
